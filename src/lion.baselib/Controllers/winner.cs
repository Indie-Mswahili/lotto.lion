...
namespace LottoLion.BaseLib.Controllers
{
    public class WinnerReader
    {
        ...
        {
            //var _site = @"http://www.645lotto.net/Confirm/number.asp?sltSeq=" + sequnce_no;  // 2013-12-04 웹사이트 변경 됨
            //var _site = @"http://www.nlotto.co.kr/lotto645Confirm.do?method=byWin&drwNo=" + sequnce_no;
            var _site = @"http://www.nlotto.co.kr/gameResult.do?method=byWin&drwNo=" + sequnce_no;
            ...
            using (var _client = new HttpClient())
            {
                ...
                if (_result.DocumentNode == null)
                    throw new Exception($"'{_site}'에서 {sequnce_no} 회차 정보를 읽는 중 오류가 발생 하였습니다.");
            }
        ...
        private List<string> GetWinnerBall(HtmlAgilityPack.HtmlDocument html_document, int sequnce_no)
        {
            ...
            var winner_division = _divs
                            .Where(_d => _d.Attributes.FirstOrDefault(_a => _a.Name == "class" && _a.Value == "lotto_win_number mt12") != null)
                            .FirstOrDefault();
            ...
            var _seqno = winner_division
                            .ChildNodes.FirstOrDefault(x => x.Name == "h3")
                            .ChildNodes.FirstOrDefault(x => x.Name == "strong")
                            .InnerText;

            if (String.IsNullOrEmpty(_seqno) || Convert.ToInt32(_seqno) != sequnce_no)
            ...
            {
                if (_pnode.Name == "p" && _pnode.Attributes[0].Name == "class" && _pnode.Attributes[0].Value == "number")
                {
                    foreach (var _node in _pnode.ChildNodes)
                    {
                        if (_node.Name == "img" && _node.Attributes[0].Name == "src")
                        {
                            if (_node.Attributes[1].Name == "alt")
                            {
                                var _b = _node.Attributes[1].Value;
                        ...
                        if (_node.Name == "span" && _node.Attributes[0].Name == "class" && _node.Attributes[0].Value == "number_bonus")
                        {
                            foreach (var _node2 in _node.ChildNodes)
                            {
                                if (_node2.Name == "img" && _node2.Attributes[0].Name == "src")
                                {
                                    if (_node2.Attributes[1].Name == "alt")
                                    {
                                        var _b = _node2.Attributes[1].Value;
        ...
        private List<string> GetWinnerInfo(HtmlAgilityPack.HtmlDocument html_document, int sequnce_no)
        {
            ...
            var _mt40 = _tables
                   .Where(_d => _d.Attributes.FirstOrDefault(_a => _a.Name == "class" && _a.Value == "tblType1 f12 mt40") != null)
                   .FirstOrDefault();
