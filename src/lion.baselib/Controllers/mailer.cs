...
namespace LottoLion.BaseLib.Controllers
{
    public class MailSenderLottoLion
    {
        ...
        public void SendLottoZipFile(string zip_file_path, TChoice choice, TbLionWinner winner, TbLionMember member, TbLionFactor curr_factor, TbLionFactor last_factor, TbLionJackpot last_jackpot)
        {
            var _message = new MimeMessage();
            {
                ...
                _message.Subject = $"[시범서비스] {member.LoginName} 님을 위한 [{choice.sequence_no}]회차 로또645 예측 번호 제공 {(member.IsDirectSend ? "D" : "I")}";
            }
            ...
            var _builder = new BodyBuilder();
            {
                _builder.HtmlBody = $"<div>"
                                    + $"<h4>안녕하세요 '{member.LoginName}'님, '로또645 예측 번호 제공 [시범 서비스]' 입니다.</h4>"
                                    + $"<p></p>"
                                    ...
                                    + $"<li> <a href='https://play.google.com/store/apps/details?id=kr.co.odinsoftware.LION'>로또사자(LottoLion)</a> "
            ...
            {
                __smtp_direct.SendMail(_message);
            }
        }
    }
}